...
def main():
    ...
    subs = glob(f"{subFolder}/sub*") ; subs.sort() ; subs = [sub.split("/")[-1] for sub in subs]
    for sub in subs:
    ...
    def alignFunc(sub='', arrayJobs={}):
        ...
        funcTemplate = saveMiddleVolumeAsTemplate(sub=sub, funcFolder=f"{subFolder}/{sub}/func/", transformFolder=transformFolder)  # 把 第一个 run 的中间的volume作为 模板
