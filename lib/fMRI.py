...
class fMRI_Time_Subjs_Embed_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, patient_ids, data_path, embed_path, TRs,
        ...
        timeseries = np.concatenate(timeseries, axis=0) # 这里形成的 timeseries 是一个 被试个数 x TRs长度 x 体素个数   或者    被试个数 x TRs长度 x 体素x坐标 x 体素y坐标 x 体素z坐标
        ...
        if data_3d:
            timeseries = np.reshape(timeseries, [len(patient_ids), int(timeseries.shape[0] / len(patient_ids)),
                                                 ...
                                                 timeseries.shape[3]])  # 被试个数 x TRs长度 x 体素x坐标 x 体素y坐标 x 体素z坐标。 shape [m x T x HWD]
            path = os.path.join(data_path, f"template_nii.pkl")  # 假如不是3D数据的话，就不需要模板或面具。因此这里需要mask。 not 3d data no need for template or mask
            self.template_nii = pickle.load(open(path, 'rb'))

            path = os.path.join(data_path, f"mask.npy")
            self.mask = np.load(path)
        else:
            timeseries = np.reshape(timeseries, [len(patient_ids), int(timeseries.shape[0] / len(patient_ids)), -1]) # 被试个数 x TRs长度 x 体素个数
