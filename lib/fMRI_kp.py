...
class fMRIAutoencoderDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, patient_ids, data_path, TRs, data_3d=True, data_name_suffix='early_visual_sherlock_movie.npy'):
        ...
        for patient_id in patient_ids:
            if data_3d:
                ...
                path = glob(f"{data_path}/sub{patient_id:03}_test*");
                assert len(path) == 1;
                path = path[0]
                [brain_t, behavior_t] = load_obj(path)
                runID_t = np.asarray(list(behavior_t['runID']))
                behav_t = np.asarray(list(behavior_t['behav']))
                timeseries.append(brain_t)
                behavs.append(behav_t)
                runID.append(runID_t)

        self.timeseries = timeseries  # np.concatenate(timeseries, axis=0)
        self.behavs = behavs
        self.runID = runID
        self.TRs = TRs
    ...
    def __getitem__(self, idx):
        ...
        return self.timeseries[idx], self.behavs[idx], self.runID[idx]
...
class fMRI_Time_Subjs_Embed_Dataset(torch.utils.data.Dataset):
    """
        储存 T x m x HWD 或 Txm（1d跨度）形状的多病人时间序列和相同时间病人序列的流形嵌入。
        T：时间长度，
        m：患者数量，
        HWD：体素在 3D HxWxD 中的位置。
    ...
    def __init__(self, patient_ids, data_path, embed_path, TRs,
                 emb_name_suffix='early_visual_SRM_10dimensions_PHATE.npy', data_3d=True,
                 data_name_suffix='early_visual_sherlock_movie.npy'):
        """
            这将整个数据集加载到内存中。这不被建议用于训练神经网络，但它更快。
            用整个全脑数据集训练需要大约140GB的内存。
        """
        # This loads the entire dataset into memory. This is not
        # recommended for training neural networks but it is faster.
        # Training with the entire full-brain dataset takes ~140GB
        ...
        runID = []
        for patient_id in patient_ids:
            # 把病人patient_id 加载数据到内存中
            print(f"loading data from patient {patient_id} into memory")
            if data_3d:  # 如果采用3D的状态
                path = os.path.join(data_path, f"patient{patient_id}.npy")
                timeseries.append(np.load(path)[TRs])
            else:  # 如果不采用3D的状态
                # path = os.path.join(data_path, f"sub{patient_id:03}_{data_name_suffix}")
                path = os.path.join(data_path, f"sub{patient_id:03}_train_{TRs}.pkl")
                [brain_t, behavior_t] = load_obj(path)
                runID_t = np.asarray(list(behavior_t['runID']))
                behav_t = np.asarray(list(behavior_t['behav']))
                timeseries.append(brain_t)
        ...
        timeseries = np.concatenate(timeseries,
                                    axis=0)  # 这里形成的 timeseries 是一个 被试个数 x TRs长度 x 体素个数   或者    被试个数 x TRs长度 x 体素x坐标 x 体素y坐标 x 体素z坐标
        behavs = np.concatenate(behavs, axis=0)
        runID = np.concatenate(runID, axis=0)

        # 在此分离，如果为自动编码器准备3D卷积层或MLP的数据集。 here separates if prepare dataset for autoencoder with 3d convolutional layer or mlp
        if data_3d:
            timeseries = np.reshape(timeseries, [len(patient_ids), int(timeseries.shape[0] / len(patient_ids)),
                                                 timeseries.shape[1], timeseries.shape[2],
                                                 timeseries.shape[
                                                     3]])  # 被试个数 x TRs长度 x 体素x坐标 x 体素y坐标 x 体素z坐标。 shape [m x T x HWD]
            path = os.path.join(data_path,
                                f"template_nii.pkl")  # 假如不是3D数据的话，就不需要模板或面具。因此这里需要mask。 not 3d data no need for template or mask
            self.template_nii = pickle.load(open(path, 'rb'))
            ...
            timeseries = np.reshape(timeseries, [len(patient_ids), int(timeseries.shape[0] / len(patient_ids)),
                                                 -1])  # 被试个数 x TRs长度 x 体素个数
            behavs = np.reshape(behavs, [len(patient_ids), -1])
            runID = np.reshape(runID, [len(patient_ids), -1])
            print(f"input data shape: {timeseries.shape}")
            # shape m x T

        # 加载PHATE形成的隐藏层的嵌入。来自于 phate.PHATE(n_components=args.zdim)
        for patient_id in patient_ids:
            path = os.path.join(embed_path, f"sub{patient_id:03}_{emb_name_suffix}.pkl")
            # embeds.append(np.load(path)[TRs]) # 新的嵌入是以分裂的方式进行的。 The new embedding is in splits
            embeds.append(load_obj(path))
        embeds = np.concatenate(embeds, axis=0)
        print('read in embeddings, shape before reshaping', embeds.shape)  # 在 嵌入中读取，在reshape之前的shape
        embeds = np.reshape(embeds,
                            [len(patient_ids), int(embeds.shape[0] / len(patient_ids)), -1])  # 被试个数 x TS长度 x 维度个数
        print(f"embedding data shape: {embeds.shape}")  # 嵌入数据形状

        self.timeseries = np.swapaxes(timeseries, 0, 1)  # shape [T x m x HWD]
        self.behavs = np.swapaxes(behavs, 0, 1)  # shape [T x m]
        self.runID = np.swapaxes(runID, 0, 1)
    ...
    def __getitem__(self, idx):
        ...
        return self.timeseries[idx], self.embeds[idx], self.behavs[idx]
